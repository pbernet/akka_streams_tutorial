...
// TODO MLLP level: is NAK generated by HAPI? Is next message sent if one is NAK?
// TODO Add not happy paths: eg ex thrown on server, ex thrown on client
// Optional: Add HL7 over HTTP: https://hapifhir.github.io/hapi-hl7v2/hapi-hl7overhttp/index.html
object TcpEchoMllp extends App {
  val logger: Logger = LoggerFactory.getLogger(this.getClass)
  val system = ActorSystem("TcpEchoMllp")
  ...
  def server(system: ActorSystem, address: String, port: Int): Future[Tcp.ServerBinding] = {
    ...
    val hl7Parser = Flow[String]
      .map(each => {
        ...
        if (StringUtils.isNoneEmpty(scrubbed)) {
    ...
    val handler = Sink.foreach[Tcp.IncomingConnection] { connection =>
      val serverEchoFlow = Flow[ByteString]
        .wireTap(each => logger.info(s"Got message from client: ${connection.remoteAddress}"))
